...
husky.games.2015 <- read.csv(file = 'data/huskies_2015.csv')


# Create a vector of the teams that the Huskies played against during that season
husky.opponents <- husky.games.2015$opponent
...
best.yards.row <- husky.games.2015[most.combined.yards, ]
print(best.yards.row)
...
MostYardsScore <- function(games){
  most.combined.yards <- max(games$combined) 
  best.yards.row <- husky.games.2015[most.combined.yards, ]
  print(best.yards.row)
}
